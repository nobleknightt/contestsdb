...
def fetch_atcoder_contests() -> list[dict]:
    ...
    if response.ok:
        ...
        for row in table[1:]:
            ...
            url = f"https://atcoder.jp{link.get('href')}"

            time_text = cols[0].find("time").text.strip()
            try:
                start_time = datetime.strptime(time_text, "%Y-%m-%d %H:%M:%S%z")
            except Exception:
                start_time = datetime.strptime(time_text, "%Y-%m-%d(%a) %H:%M")
                start_time = start_time.replace(tzinfo=ZoneInfo("Asia/Tokyo"))

            start_time = start_time.astimezone(ZoneInfo("Asia/Kolkata"))

            hours, minutes = map(int, cols[2].text.strip().split(":"))
    ...
    if response.ok:
        ...
        for data in contests_data:
            ...
            start_time = datetime.fromtimestamp(
                float(data["startTimeSeconds"]), tz=ZoneInfo("Asia/Kolkata")
            )
...
def fetch_geeksforgeeks_contests() -> list[dict]:
    ...
    payload = {"type": "contest", "sub_type": "upcoming"}
...
def fetch_leetcode_contests() -> list[dict]:
    ...
    if response.ok:
        ...
        for data in contests_data:
            ...
            start_time = datetime.fromtimestamp(int(data["startTime"])).astimezone(
...
weekday, month, day, year, hours, minutes, ampm = (
    datetime.now().strftime("%a %b %d %Y %I %M %p").split()
)
...
readme_content[4] = (
    f"*Contest DB last updated at {weekday}, {month} {day}, {year} {hours}:{minutes} {ampm}*  \n"
)
